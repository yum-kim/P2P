...
3. **채팅 화면**
    ...
    3-1. 채팅 리스트
      - 이전 채팅했던 리스트가 나열되며, 클릭 시 오른쪽에 채팅창이 열리며 이전 채팅 내역을 불러온다.
      - 기존에 없던 상대와의 채팅이 추가될 경우, 리스트를 새로 불러온다.
    
    3-2. 채팅창
      - socket.io를 통해 message 이벤트를 구현하였으며, 새로운 message 이벤트 수신 시 채팅창을 업데이트한다.
      - 프로필 모달 내의 'Message' 버튼 혹은 채팅 리스트에서 직접 클릭을 통해 진입할 수 있다.
      - 기본적으로 20개 단위로 채팅 내역을 불러오며, 스크롤 페이징을 통해 이전 내역을 불러온다.
      - 채팅 시간은 당일 채팅 내역의 경우 시간만 표기, 당해 당일 이전의 경우 날짜/시간 표기, 해가 다를 경우 연도/날짜/시간을 함께 표기한다.
      - 뒤로가기 버튼 클릭 시 채팅창이 닫힌다.
